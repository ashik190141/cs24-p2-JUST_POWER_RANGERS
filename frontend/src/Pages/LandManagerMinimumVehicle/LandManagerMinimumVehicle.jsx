...
const LandManagerMinimumVehicle = () => {
    ...
    useEffect(() => {
        fetch(
          `http://localhost:5000/minimum-vehicle-and-cost/${user?.email}`
        )
            .then(res => res.json())
  ...
  if (trucks.length > 0) {
   // Declare subsets variable
    ...
    function findSubsets(idx, target, current, trucks) {
      ...
      findSubsets(idx - 1, target - trucks[idx].capacity, current, trucks);
      current.pop();
    }
    ...
    const newTarget = findNextSubsetSum(trucks.length, arr, target, prefixSum);
